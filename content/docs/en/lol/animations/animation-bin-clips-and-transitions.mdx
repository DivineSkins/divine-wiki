...
[Exporting a `.anm`](/docs/lol/animations/animation-import-export) is only a part of animation modding. The other half is the animation bin, which decides **which** clip plays and **when**. This guide covers the clip types you will meet, how to switch between clips based on game state, and how to add transitions so animations stop snapping.
...
`ParallelClipData` depends on [animation masks](/docs/lol/animations/animation-masks) to decide which joints each of the two clips is allowed to drive.
...
| `SubmeshVisibilityBoolDriver`           | Whether a [submesh](/docs/lol/vfx-bins/submesh-visibility) is currently shown |
...
**Submeshes and VFX** are switched in the skin bin, by `PersistentEffectConditions`, driven by a `*MaterialBoolDriver`. That mechanism cannot switch an animation, so do not go looking for a clip field on it. It is covered on [its own page](/docs/lol/vfx-bins/persistent-effect-conditions).
...
> **Note:** A transition that fires but tears visibly is a different problem. Check the [frame rate](/docs/lol/animations/animation-frame-rate) of both clips involved.
