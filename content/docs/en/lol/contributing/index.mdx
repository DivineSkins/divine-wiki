...
2. Look for the **Edit on GitHub** link. It sits in the right-hand panel on desktop, or near the bottom of the page on mobile. Click it.
3. On GitHub, click the pencil icon (top right of the file).
4. Make your changes.
5. If you don't have write access, GitHub will offer to fork the repo for you. Accept it and keep going.
6. Scroll down. Pick "Create a new branch for this commit and start a pull request".
...
6. Make a branch:
   ...
   ```
7. Make your changes (see [Add a new guide](#add-a-new-guide) below).
8. Commit, push, open a pull request to `main`.

## Add a new guide

A guide is one `.mdx` file under `content/docs/en/lol/<category>/`.

1. Pick a category (e.g. `tools`, `maya`, `blender`).
2. Create a file like `content/docs/en/lol/tools/my-tool.mdx`. Use kebab-case for the filename.
...
4. Run `npm run dev` and confirm your page renders at `/en/docs/lol/<category>/<slug>`.
5. Commit and open a PR.

New pages show up in the sidebar on their own, at the end of the category. Want a different spot? Open `content/docs/en/lol/<category>/meta.json` and add your file name to the `pages` array in the position you want.
...
- Every image needs an `alt` attribute. Reviewers check this.
- Keep each file under **500 KB**. Bigger files slow the site build.
- PNG for screenshots, JPG for photos. WebP is fine too.

## What happens after you open a PR

1. Cloudflare builds a preview. It posts a link as a comment on your PR. Open it to see your changes live.
2. CI checks code formatting. Fix anything it flags.
3. A maintainer reviews voice, links, and image alt text. They may ask for small changes.
...
- **Stuck on Git or the editor:** ask in `#wiki-feedback` on Discord.
