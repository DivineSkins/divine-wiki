...
**The mask side** is a texture whose colour channels drive the animation: two of them supply moving noise, and one of them decides where that noise is allowed to apply.
...
And one you want explicitly set to false: `UI_SECONDARY_ON`. That switch draws the mask texture as visible colour. You do not want to see the mask, you only want it read as a driver.
