...
> **This repo (Divine Skins wiki):** content is MDX, not plain markdown. The blank-line rules below also apply between JSX components (`<Callout>`, `<Tabs>`, etc.) and markdown content. Repo-specific quirks override generic rules: H1 comes from frontmatter `title` (start content at `##`), escape `<` before digits (`\<3`), and Prettier is the formatter of record (`npm run format`), not markdownlint. See CLAUDE.md "MDX quirks".
